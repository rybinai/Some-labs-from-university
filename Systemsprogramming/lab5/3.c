/*
 * lab5 - 3.c - Измерьте время работы 1.c (Без буферизации)
 *
...
int main(int argc, char *argv[])
{
    ...
    for (size_t i = 0; i < num_sizes; i++) {
        ...
        setvbuf(copyFile, NULL, _IONBF, 0);
        ...
        printf
            ("Время выполнения при размере буфера %zu байт: %ld микросекунд (Без буферизации)\n",
             buffer_size, elapsed);
    }
